...
Kamal uses [dotenv](https://github.com/bkeepers/dotenv) to automatically load environment variables set in the `.env` file present in the application root. This file can be used to set variables like `KAMAL_REGISTRY_TOKEN` or database passwords. But for this reason you must ensure that .env files are not checked into Git or included in your Dockerfile! The format is just key-value like:

```bash
KAMAL_REGISTRY_TOKEN=my-token
...
KAMAL_REGISTRY_TOKEN=<%= `op read "op://Vault/Docker Hub/password" -n --session  #{session_token}` %>
...
```yaml
volumes:
  - "/local/path:/container/path" # Local volume
  - "data:/container/path" # With named volume
```
